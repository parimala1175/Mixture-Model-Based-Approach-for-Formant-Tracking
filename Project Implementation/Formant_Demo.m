...
close all;
clc;
fid=fopen('si1189.fb', 'r', 'b');
n_frame=fread(fid, 1, 'int32');
samPeriod=fread(fid, 1, 'int32');
sampSize=fread(fid, 1, 'int16');
numComps=sampSize/4; 			 
fileType=fread(fid, 1, 'int16');
n_frame=50; %% here the no of frames should be mentioned
dataSet=zeros(n_frame,numComps);
%% this reads the  formant values from the VTR fb files
%% this acts as the reference for comparison
for n = 1:n_frame
...
legend('R-Mixture model approach','G-Original')
B=mean_matrix';
x=0;
p=0;
%% computing the error at the end
for row=1:1:size(dataSet,1)
    for j=1:1:4
        x=x+(dataSet(row,j)-B(row,j))^2;
        p=p+1;
    end
end
